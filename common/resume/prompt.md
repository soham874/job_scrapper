...
`<<LOCATION>>`, `<<JOB_DESCRIPTION>>`, `<<BASE_RESUME>>`.
...
- Reorder or trim the skills list to lead with skills the posting names, **but
  only skills already present in the base resume**.
...
- Drop a bullet entirely when it is irrelevant to this posting and the resume
  needs the space.
- Rewrite the summary or objective line, if the base resume has one, to
  emphasise the parts of the candidate's real background this posting cares
  about.
...
- Return a **complete, compilable LaTeX document** — everything from
  `\documentclass` through `\end{document}`.
- Keep the base resume's document class, packages, custom macros, spacing
  commands, and overall visual structure exactly as they are. You are editing
  the content between the structural commands, not the layout.
...
- Do not touch the preamble at all: `\documentclass`, `\usepackage` lines,
  `\input{glyphtounicode}`, `\pdfgentounicode=1`, `\titleformat`, `\newcommand`
  definitions, and page-geometry settings carry over byte-for-byte.
...
- Keep the resume to the same page count as the base — if trimming is needed to
  ...
  or font size.
- Output **only** the LaTeX source. No markdown code fences, no preamble like
  "Here is the tailored resume", no commentary, no explanation of your changes.
  The first characters of your response must be `\documentclass`.
